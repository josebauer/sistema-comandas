...
class ItemUsuario(ctk.CTkFrame):
  def __init__(self, master, usuario, ver_callback, editar_callback, excluir_callback, **kwargs):
    ...
    self.configure(fg_color="darkred", corner_radius=10)
    ...
    self.icone_editar = ctk.CTkImage(light_image=Image.open(os.path.join(icons_path, "pencil.png")), size=(20, 20))
    self.icone_excluir = ctk.CTkImage(light_image=Image.open(os.path.join(icons_path, "trash.png")), size=(20, 20))
    self.icone_ver = ctk.CTkImage(light_image=Image.open(os.path.join(icons_path, "view.png")), size=(20, 20))
